...
We outline a comprehensive experimental protocol to validate **DL-MoM** and report ablation-suite results in Section 6.6. Unless otherwise stated, all experiments report: **task accuracy**, **tokens generated** (explicit + latent), **wall-clock time**, **peak memory**, and **failure/drift rate**. For stochastic variants, results are averaged over **3 seeds** and reported with **mean ± std**.
...
### 6.6 Experimental Results (Ablation Suite)

We include the ablation-suite plots generated by the runner in `runs/ablations/plots`. Each plot aggregates three seeds using the protocol in Section 6.0. MiniCache in A4 corresponds to the depth-merge configuration.

#### 6.6.1 Suite Overview

![Ablation suite summary: mean vs best accuracy for A1-A4](runs/ablations/plots/suite_summary.png)

![Accuracy heatmap across experiments and seeds](runs/ablations/plots/accuracy_heatmap.png)

#### 6.6.2 Accuracy vs Compute

![Accuracy vs latent reasoning steps](runs/ablations/plots/accuracy_vs_total_steps.png)

![Accuracy vs inference time](runs/ablations/plots/accuracy_vs_wallclock.png)

#### 6.6.3 Drift and Stability

![Distribution of KL drift from the reference pipeline](runs/ablations/plots/drift_histogram.png)

#### 6.6.4 A1 Soft Token Variants

![A1 soft token variant comparison](runs/ablations/plots/a1_soft_mode_comparison.png)

![A1 variance across seeds](runs/ablations/plots/a1_variance_analysis.png)

#### 6.6.5 A2 Controller Variants

![A2 controller variant comparison](runs/ablations/plots/a2_controller_comparison.png)

![A2 entropy traces by controller variant](runs/ablations/plots/a2_entropy_traces.png)

![A2 switches vs accuracy trade-off](runs/ablations/plots/a2_switches_vs_accuracy.png)

![CDF of latent and explicit mode switches](runs/ablations/plots/switch_count_cdf.png)

#### 6.6.6 A3 Communication Protocols

![A3 communication protocol efficiency](runs/ablations/plots/bandwidth_vs_accuracy.png)

#### 6.6.7 A4 KV Compression

![A4 KV compression impact on accuracy](runs/ablations/plots/a4_accuracy.png)

![A4 accuracy vs latent exploration trade-off](runs/ablations/plots/a4_accuracy_vs_exploration.png)

![A4 distribution shift under compression](runs/ablations/plots/a4_kl_drift.png)

![A4 compression effect on latent exploration](runs/ablations/plots/a4_latent_exploration.png)

![A4 runtime by compression method](runs/ablations/plots/a4_runtime.png)

![A4 compression effect on expert switching](runs/ablations/plots/a4_switches.png)

![A4 compression impact on throughput](runs/ablations/plots/memory_vs_throughput.png)
...
* **Evaluation scope.** This paper provides architecture + protocol + reproducibility tooling and reports ablation-suite results; full multi-benchmark coverage remains ongoing.
